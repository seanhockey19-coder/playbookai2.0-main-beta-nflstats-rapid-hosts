...
const HOST = process.env.RAPIDAPI_NBA_HOST || "";

async function raFetch<T>(path: string): Promise<T> {
  if (!RAPID_KEY || !HOST) throw new Error("RapidAPI NBA not configured");
...
export const RapidApiNbaProvider: Provider = {
  name: "rapidapi:nba",
  async healthy() {
  ...
  async getPropsForGame(sport: SportKey, gameId: string): Promise<Prop[]> {
    ...
    return (rows as any[]).map((p) => ({
      id: String(p.id || `${p.market}:${p.player}:${p.book || "ra-nba"}`),
      sport,
      gameId,
